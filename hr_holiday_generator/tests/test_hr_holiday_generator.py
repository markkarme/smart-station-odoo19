...
class TestHrHolidayGenerator(TransactionCase):
    ...
    def test_action_generate_uses_arabic_language(self):
        """Test Arabic holiday names are enriched from Nager.Date."""
        arabic = self.env.ref('base.lang_ar')
        if not arabic.active:
            arabic.active = True
        egypt = self.env.ref('base.eg')

        wizard = self.wizard_model.with_context(lang='ar_001').create({
            'country_id': egypt.id,
            'generation_mode': 'year',
            'year': '2026',
        })

        calendarific_response = {
            'response': {
                'holidays': [
                    {
                        'name': 'Revolution Day January 25',
                        'description': 'Revolution Day January 25 is a national holiday in Egypt',
                        'date': {'iso': '2026-01-29'}
                    }
                ]
            }
        ...
        def mock_get(url, timeout=30, params=None):
            response = type('Response', (), {})()
            response.status_code = 200
            if 'calendarific.com' in url:
                response.json = lambda: calendarific_response
            elif 'nager.at' in url:
                response.json = lambda: nager_response
            else:
                response.json = lambda: {
                    'responseData': {
                        'translatedText': 'وصف العطلة',
                    }
                }
            return response
        ...
        with patch('requests.get', side_effect=mock_get):
            ...
            self.assertEqual(wizard.calender_leaves_ids[0].name, 'عيد الثورة 25 يناير')
            self.assertEqual(
                wizard.calender_leaves_ids[0].description, 'وصف العطلة')

    def test_action_generate_translates_english_only_holidays(self):
        """Test remaining English holidays are translated to Arabic."""
        arabic = self.env.ref('base.lang_ar')
        if not arabic.active:
            arabic.active = True
        egypt = self.env.ref('base.eg')
        ...
        calendarific_response = {
            'response': {
                'holidays': [
                    {
                        'name': 'Ramadan Start',
                        'description': 'Ramadan is a period of prayer and fasting.',
                        'date': {'iso': '2026-02-19'}
                    }
                ]
            }
        ...
        def mock_get(url, timeout=30, params=None):
            ...
            if 'calendarific.com' in url:
                ...
                translations = {
                    ...
                    'Ramadan is a period of prayer and fasting.':
                        'رمضان فترة من الصلاة والصيام.',
                }
                response.json = lambda: {
                    'responseData': {
                        'translatedText': translations.get(query, query),
                    }
                }
            return response
        ...
        with patch('requests.get', side_effect=mock_get):
            ...
            self.assertEqual(wizard.calender_leaves_ids[0].name, 'بداية رمضان')
            self.assertEqual(
                wizard.calender_leaves_ids[0].description,
                'رمضان فترة من الصلاة والصيام.')
