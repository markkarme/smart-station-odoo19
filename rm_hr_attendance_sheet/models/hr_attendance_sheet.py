...
class AttendanceSheet(models.Model):
    ...
    def _cron_generate_attendance_sheet(self):
        ...
        version_ids = self.env['hr.version'].search(
            [('is_in_contract', '=', True), ('auto_attendance_sheet', '=', True),
             ('att_policy_id', '!=', False)])
        employee_ids = version_ids.mapped('employee_id')
    ...
    def onchange_employee(self):
        ...
        version_id = employee._get_version(date_from)
        if not version_id or not version_id.is_in_contract:
            raise ValidationError(
                _('There Is No Valid Contract For Employee %s' % employee.name))
        self.contract_id = version_id
    ...
    def get_attendances(self):
        for att_sheet in self:
            ...
            tz = pytz.timezone(emp_tz)
            calendar_id = (att_sheet.contract_id and att_sheet.contract_id.resource_calendar_id) or emp.resource_calendar_id
            if not calendar_id:
                raise ValidationError(_(
    ...
    def action_create_payslip(self):
        ...
        for sheet in self:
            version_id = sheet.employee_id._get_version(sheet.date_from)
            if not version_id:
            ...
            payslip_name = version_id.structure_type_id.default_struct_id.payslip_name or _('Salary Slip')
            ...
            payslip_id = payslip_obj.create({
                ...
                'struct_id': version_id.structure_type_id.default_struct_id.id,
