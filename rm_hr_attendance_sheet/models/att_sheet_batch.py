...
class AttendanceSheetBatch(models.Model):
    ...
    def onchange_employee(self):
        ...
        ttyme = datetime.combine(fields.Date.to_date(date_from), time.min)
    ...
    def gen_att_sheet(self):
        ...
        for batch in self:
            ...
            for employee in employee_ids:
                ...
                version_id = employee._get_version(from_date)
                if not version_id or not version_id.is_in_contract:
                    raise UserError(_(
                        "There is no running contract version for: %s" % employee.name))
