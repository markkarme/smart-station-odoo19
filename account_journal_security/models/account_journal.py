...
class AccountJournal(models.Model):
    ...
    journal_restriction = fields.Selection(
        [("none", "None"), ("modification", "Modification"), ("total", "Total")],
    ...
    def _check_journal_users_restriction(self, field):
        ...
        if self.modification_user_ids and self.user_ids:
            raise ValidationError(
                _(
                    'You cannot set values in "Totally allowed to" and '
                    '"Modifications allowed to" at the same time. These options '
        ...
        for rec in self.sudo():
            ...
            if journal_users and env_user not in journal_users:
                raise ValidationError(
                    _(
                        'You cannot restrict the journal "%s" to users without '
                        "including yourself, as you would no longer be able to "
                        "see this journal"
                    )
                    % (rec.name)
                )
